...
ROUTE_HOST=$(echo $PROJECT_NAME$EXTERNAL_REGISTRY | awk '{gsub("default-route-openshift-image-registry","");print $1}')
...
mvn clean package
echo Loki_%47 | sudo -S ${BUILD_TOOL}  build --platform=linux/amd64 --pull -t ${IMAGE_PREFIX_EXTERNAL}/acmeair-mainservice-java:latest --no-cache -f ${DOCKERFILE} .
...
kubectl delete -f ${MANIFESTS}
mvn clean package
echo Loki_%47 | sudo -S ${BUILD_TOOL}  build --platform=linux/amd64 --pull -t ${IMAGE_PREFIX_EXTERNAL}/acmeair-authservice-java --no-cache -f ${DOCKERFILE} .
...
kubectl delete -f ${MANIFESTS}
mvn clean package
echo Loki_%47 | sudo -S ${BUILD_TOOL}  build --platform=linux/amd64 --pull -t ${IMAGE_PREFIX_EXTERNAL}/acmeair-bookingservice-java --no-cache -f ${DOCKERFILE} .
...
kubectl delete -f ${MANIFESTS}
mvn clean package
echo Loki_%47 | sudo -S ${BUILD_TOOL}  build --platform=linux/amd64 --pull -t ${IMAGE_PREFIX_EXTERNAL}/acmeair-customerservice-java --no-cache -f ${DOCKERFILE} .
...
kubectl delete -f ${MANIFESTS}
mvn clean package
echo Loki_%47 | sudo -S ${BUILD_TOOL}  build --platform=linux/amd64 --pull -t ${IMAGE_PREFIX_EXTERNAL}/acmeair-flightservice-java --no-cache -f ${DOCKERFILE} .
